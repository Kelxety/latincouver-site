...
function ProfileContainer() {
  ...
  let employee_profile = useMemo(() => {
    return {
      ...
      medical_condition:
        profile_data?.medical_condition,
  ...
  const {
    isLoading: job_title_loading,
    data: job_title_res_data,
  } = useQuery('jobtitleData', () =>
    api
      .get(`${JOB_TITLES}`)
      .then((res) => res.data)
  );
  ...
  const job_titles = useMemo(() => {
    return job_title_res_data?.results.map(
      (data: any) => {
        return {
          value: data?.id,
          label: data?.name
            .toLowerCase()
            .replace(/\b\w/g, (s: string) =>
              s.toUpperCase()
            ),
        };
      }
  ...
  const departments_api_data = useMemo(() => {
    return department_res_data?.results?.map(
      (data: any) => {
        return {
          value: data?.id,
  ...
  const profile_department: any[] =
    useMemo(() => {
      const depart_list: any[] = [];

      for (
        let x = 0;
        x < employee_profile?.department?.length;
        x++
      ) {
        const department =
          departments_api_data?.find(
            (obj: any) =>
              obj.label ===
  ...
  const onChangeMedicalCondition = (
    value: string
  ) => {
    setEmployeeProfileData((prevState: any) => ({
      ...prevState,
  ...
  const { mutateAsync: updateEmployee } =
    useMutation({
      mutationFn: () =>
        api
          .put(
            `${HR_EMPLOYEES}${employee_profile_data?.pk}/`,
            employee_profile_data
          )
          .then((res) => res.data),
      ...
      onSuccess: () => {
        ...
        const description: string =
          'This Employee profile has been updated successfully!';
      ...
      onError: (error, variables, context) => {
        ...
        const description: string =
          "There's something wrong when updating this record. try again.";
        openNotificationWithIcon(
          'error',
          message,
          description
  ...
  return (
    <ProfileView
      ...
      departments_api_data={departments_api_data}
      onChangeIsManager={onChangeIsManager}
